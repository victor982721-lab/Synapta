...
Este documento complementa el AGENTS general de Neurologic y la Política cultural y de calidad. Define las reglas específicas para **{{PROJECT_NAME}}**. Sustituye los placeholders y elimina las notas una vez completado.

## 1. Entorno

* **Sistema operativo objetivo:** Windows 10 (mínimo).
* **Shell principal:** PowerShell 7.5.x (`pwsh`).
* **Lenguajes permitidos:** {{PRIMARY_LANGUAGES}}.
* **Target frameworks (si aplica):** {{TARGET_FRAMEWORKS}}. Mantén multi-target `net8.0;net7.0;net6.0` salvo justificación documentada.

## 2. Propósito del proyecto

Describe brevemente la finalidad de {{PROJECT_NAME}} dentro del ecosistema Neurologic, indicando si genera artefactos reutilizables, una aplicación final o ambos.
...
1. Lista explícita de módulos de Core o scripts compartidos que este proyecto debe reutilizar o producir. Ejemplo:
   ...
   * `{{PROJECT_NAME}}.Scripts.Validate` – Nueva utilidad solicitada en `csv/artefacts.csv`.

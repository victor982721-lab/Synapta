...
function Test-ProjectStructure {
    ...
    $requiredItems = @('AGENTS.md','README.md','docs','csv')
    ...
    if (Test-Path $docsPath) {
        $docsRequired = @('solicitud_de_artefactos.md','filemap_ascii.txt','table_hierarchy.json','plan.md')
        foreach ($doc in $docsRequired) {
            $docPath = Join-Path $docsPath $doc
            if (-not (Test-Path $docPath)) {
                Write-Warning "$ProjectName: falta docs/$doc"
                $ok = $false
            }
        }
    }

    $csvPath = Join-Path $ProjectPath 'csv'
    if (Test-Path $csvPath) {
        $csvRequired = @('modules.csv','artefacts.csv')
        foreach ($csv in $csvRequired) {
            $csvFile = Join-Path $csvPath $csv
            if (-not (Test-Path $csvFile)) {
                Write-Warning "$ProjectName: falta csv/$csv"
                $ok = $false
            }
        }
    }
